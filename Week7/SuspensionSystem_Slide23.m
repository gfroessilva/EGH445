%% Sinusoidal Tracking with state feedback controller with integral action
clear
LineWidth = 5;
FontSize = 30;
...
for k = 1:(N-1)
    ...
    u_total = -K_aug(1:n_states) * x_current + u_ff(k) - K_aug(n_states+1:end) * q_current;
...
f(9) = figure(9); f(9).Theme = 'light'; f(9).Color = 'white'; hold off
hold off
