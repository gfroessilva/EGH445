%% Non-zero state Regulation (and parameter uncertainty) through integral action 
% The system is regulated to a non zero setpoint 
% using a state feedback controller with integral action.
...
% The closed-loop system is then simulated with a step input (reference) and the integral action is applied.
...
r = zeros(size(t));
r(t >= t(end)/2) = .5; % step disturbance at t=1
...
for k = 1:(N-1)
    ...
    x_next = sysD.A * x_current + sysD.B * u_total;% + Bd_d * r(k);
...
f(6) = figure(6); f(6).Theme = 'light'; f(6).Color = 'white'; hold off
hold off
stairs(t, y_history, 'b', 'LineWidth', LineWidth);
hold on 
stairs(t, r, 'k--', 'LineWidth', LineWidth);
legend('Wheel Position', 'Reference')
